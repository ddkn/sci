...
# Constants
MG_TO_G             = 1E-3 # 1g/1000mg
CM_TO_NM            = 1E7  # 1E7nm/1cm
MIN_TO_SEC          = 60   # 60sec/1min
ROT_EST_SCALE       = 30
...
class SputterTarget():
    ...
    def get_rotating_table_estimate(self):
        ...
        df = self.__df[headers]
        df.columns = ['rotating est. ' + col for col in df.columns]

        return df.apply(lambda x: x/ROT_EST_SCALE)
